...
const config = {
  ...
  validate() {
    ...
    if (this.getRowsCount() < 10 || this.getRowsCount() > 30) {
      ...
      result.errors.push(
        "Неверные настройки.Значение должно быть в диапазоне [16,30]"
      );
    }
    ...
    if (this.getSpeed() < 1 || this.getSpeed() > 10) {
      result.isValid = false;
      result.errors.push(
        "Неверные настройки.Значение должно быть в диапазоне [1,10]"
      );
    }

    if (this.getMinFoodCount() < 5 || this.getMinFoodCount() > 50) {
      result.isValid = false;
      result.errors.push(
        "Неверные настройки.Значение должно быть в диапазоне [5,50]"
      );
    }
...
const game = {
  ...
  getRandomFreeCoordinates() {
    ...
    while (true) {
      ...
      if (
        !exclude.some((exPoint) => (rndPoint.x === exPoint.y) === exPoint.y)
      ) {
        return rndPoint;
      }
    }
  ...
  canMakeStep() {
    ...
    return (
      ...
      nextHeadPoint.x < this.config.getColsCount() && // Задание!
